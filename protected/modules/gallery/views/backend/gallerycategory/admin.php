...
$this->widget('CustomGridView', array(
	...
	'columns'=>array(
		...
		array(
			'name' => 'active',
			'type' => 'raw',
			'value' => 'Yii::app()->controller->returnStatusHtml($data, "gallery-category-grid", 1)',
		...
		array(
			...
			'buttons' => array(
				'up' => array(
					...
					'url'=>'Yii::app()->createUrl("/gallery/backend/gallerycategory/move", array("id"=>$data->id, "direction" => "up"))',
					'options' => array('class'=>'arrow_image_up'),
					'visible' => '$data->sorter > "'.$minSorter.'"',
					'click' => "js: function() { ajaxMoveRequest($(this).attr('href'), 'gallery-category-grid'); return false;}",
				),
				'down' => array(
					...
					'url'=>'Yii::app()->createUrl("/gallery/backend/gallerycategory/move", array("id"=>$data->id, "direction" => "down"))',
...
<?php
	if (GalleryCategory::model()->count() > 0) {
		$this->renderPartial('//site/admin-select-items', array(
			'url' => '/gallery/backend/gallerycategory/itemsSelected',
			'id' => 'gallery-category-grid',
			'model' => $model,
			'options' => array(
...
$str_js = "
		...
		function installSortable() {
			$('#gallery-category-grid table.items tbody').sortable({
				forcePlaceholderSize: true,
				forceHelperSize: true,
				items: 'tr',
				update : function () {
					serial = $('#gallery-category-grid table.items tbody').sortable('serialize', {key: 'items[]', attribute: 'data-bid'}) + '&{$csrf_token_name}={$csrf_token}';
					$.ajax({
						'url': '" . $this->createUrl('/gallery/backend/gallerycategory/sortitems') . "',
						'type': 'post',
						'data': serial,
